...
export default function RestaurantHome() {
  ...
  useEffect(() => {
    const isPietraViva = slug === 'pietra-viva';
    const src = isPietraViva ? '' : images.logo_url;
    if (!src) return;

    // Skip extraction for the local Pietra Viva logo to avoid CORS issues with certain browsers/servers
    if (isPietraViva) {
  ...
  return (
    <div className="bg-[#FBFBFB] dark:bg-[#1A1A1A] text-[#1A1A1A] dark:text-[#FDFCF0] font-sans min-h-screen flex flex-col antialiased">
      ...
      <div className="relative w-full h-48 sm:h-56 bg-[#008081]/10 dark:bg-[#008081]/20 overflow-hidden">
        ...
        {slug === 'demo' && (
          <Link to="/trova" className="absolute top-6 left-5 px-3 py-1.5 bg-black/40 hover:bg-black/60 backdrop-blur-md rounded-full text-white transition flex items-center gap-1 font-bold z-10 text-sm shadow-sm">
      ...
      <main className="flex-1 px-5 pb-24 relative -mt-14 z-10 overflow-y-auto no-scrollbar">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          >
            {slug === 'demo' ? (
              ...
              <div className="w-full h-full bg-[#008081]/10 rounded-full flex items-center justify-center font-sans text-xl font-black text-[#008081] p-2 text-center leading-tight">
                {restaurant?.name}
              </div>
            )}
          </div>
          ...
          <div className="flex flex-row items-center justify-center gap-4 mt-4">
            {images.phone_number && (
              <a href={`tel:${images.phone_number.replace(/\s+/g, '')}`} className="w-12 h-12 rounded-2xl flex items-center justify-center border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#252525] shadow-sm hover:shadow-md hover:border-[#008081] text-[#008081] transition-all duration-300">
                <Phone className="w-5 h-5" />
              </a>
            )}
            {images.instagram_url && (
              <a href={images.instagram_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-2xl flex items-center justify-center border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#252525] shadow-sm hover:shadow-md hover:border-[#008081] text-[#008081] transition-all duration-300">
                <Instagram className="w-5 h-5" />
              </a>
            )}
            {images.facebook_url && (
              <a href={images.facebook_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-2xl flex items-center justify-center border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#252525] shadow-sm hover:shadow-md hover:border-[#008081] text-[#008081] transition-all duration-300">
                <Facebook className="w-5 h-5" />
              </a>
            )}
            {images.tiktok_url && (
              <a href={images.tiktok_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-2xl flex items-center justify-center border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#252525] shadow-sm hover:shadow-md hover:border-[#008081] text-[#008081] transition-all duration-300">
            ...
            {images.google_maps_url && (
              <a href={images.google_maps_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-2xl flex items-center justify-center border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#252525] shadow-sm hover:shadow-md hover:border-[#008081] text-[#008081] transition-all duration-300">
        ...
        <div className="space-y-4">
          {/* Fidelity Pro Card — Prominent Top Placement */}
          ...
          {images.visibility_menu !== 'false' && (
            <Link to={`/${slug}/menu-del-giorno`} className="group relative block w-full h-40 rounded-3xl overflow-hidden shadow-premium hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
              ...
              <div className="relative h-full flex items-end justify-between p-6">
                ...
                <div className="w-8 h-8 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center group-hover:bg-gradient-to-r group-hover:from-[#008081] group-hover:to-teal-500 transition-colors duration-300 shadow-md">
                  <ChevronRight className="w-4 h-4 text-white drop-shadow-md" />
                </div>
